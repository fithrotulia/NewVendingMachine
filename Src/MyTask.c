...
//			Led_kembali500();
		...
		if(Button_Continue()){
			BuzzerOn;
			LCD_Clear();
			LCD_SetCursor(0, 0);LCD_Print("Pesanan gagal!!");
			LCD_SetCursor(0, 1);LCD_Print("Koin Kurang!!");
	...
	case dropStuff:
	{
		LCD_Clear();
		Led_Process();
		LCD_SetCursor(0, 0);LCD_Print("Minuman Keluar");
		LCD_SetCursor(0, 1);LCD_Print("Harga Minuman 1000");
		LCD_SetCursor(0, 3);LCD_Print("Thank You!");
...
//		LCD_PrintNum(Coin_temp);LCD_Print("00");
		if(++i>30){
			i=0;
			Condition=finishdelay;
		}
